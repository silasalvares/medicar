...
export class ConsultasComponent implements OnInit {
	...
	public desmarcarConsulta(consulta_id: number) {
		this.consultaService.desmarcarConsulta(consulta_id).subscribe(
			apiResponse => {
				this.messageService.add({severity: 'success', detail: 'Consulta desmarcada com sucesso'})
				this.carregarConsultas();
			}
		)
	}
