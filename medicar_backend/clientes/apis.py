...
class ConsultasAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        consultas = ConsultaService.listar_consultas_agendadas(request.user)
        serializer = ConsultaSerializer(consultas, many=True)
        return Response(serializer.data)

    def post(self, request, *args, **kwargs):
        request_data = NovaConsultaSerializer(request.data)
        nova_consulta = ConsultaService.marcar_consulta(
            request_data.data.get('agenda_id'),
            request_data.data.get('horario'),
            request.user
        )
